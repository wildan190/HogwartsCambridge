<?php
	...
	if(isset($_POST['bsimpan']))
	{
		...
		{
			//Data akan di edit
			$edit = mysqli_query($koneksi, "UPDATE grade6 set
											 	NoKK = '$_POST[tNoKK]',
											 ...
											 WHERE id_grade6 = '$_GET[id]'
										   ");
			if($edit) //jika edit sukses
			{
				echo "<script>
						alert('Edit data suksess!');
						document.location='grade6.php';
				     </script>";
			}
			else
			{
		...
		else
		{
			...
			$simpan = mysqli_query($koneksi, "INSERT INTO grade6 (NoKK, namasis, alamat, gender, agama,walsis,tgllhr)
										  VALUES ('$_POST[tNoKK]', 
										  		 '$_POST[tnamasis]', 
										  		 '$_POST[talamat]', 
										  		 '$_POST[tgender]',
	...
	if(isset($_GET['hal']))
	{
		...
		if($_GET['hal'] == "edit")
		{
			...
			$tampil = mysqli_query($koneksi, "SELECT * FROM grade6 WHERE id_grade6 = '$_GET[id]' ");
		...
		else if ($_GET['hal'] == "hapus")
		{
			...
			$hapus = mysqli_query($koneksi, "DELETE FROM grade6 WHERE id_grade6 = '$_GET[id]' ");
			if($hapus){
				echo "<script>
...
<div class="container">
	...
	<h1 class="text-center">Form Siswa Kelas 6</h1>
	...
	<div class="card mt-3">
	  ...
	  <div class="card-body">
	    ...
	    <table class="table table-bordered table-striped">
	    	...
	    	<?php
	    		$no = 1;
	    		$tampil = mysqli_query($koneksi, "SELECT * from grade6 order by id_grade6 desc");
	    	...
	    	<tr>
				...
				<td><?=$data['tgllhr']?></td>
	    		<td>
	    			<a href="grade6.php?hal=edit&id=<?=$data['id_grade6']?>" class="btn btn-warning"> Update </a><br />
	    			<a href="grade6.php?hal=hapus&id=<?=$data['id_grade6']?>" 
