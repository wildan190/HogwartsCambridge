<?php
	...
	if(isset($_POST['bsimpan']))
	{
		...
		{
			//Data akan di edit
			$edit = mysqli_query($koneksi, "UPDATE grade5 set
											 	NoKK = '$_POST[tNoKK]',
											 ...
											 WHERE id_grade5 = '$_GET[id]'
										   ");
			if($edit) //jika edit sukses
			{
				echo "<script>
						alert('Edit data suksess!');
						document.location='grade5.php';
				     </script>";
			}
			else
			{
		...
		else
		{
			...
			$simpan = mysqli_query($koneksi, "INSERT INTO grade5 (NoKK, namasis, alamat, gender, agama,walsis,tgllhr)
										  VALUES ('$_POST[tNoKK]', 
										  		 '$_POST[tnamasis]', 
										  		 '$_POST[talamat]', 
										  		 '$_POST[tgender]',
	...
	if(isset($_GET['hal']))
	{
		...
		if($_GET['hal'] == "edit")
		{
			...
			$tampil = mysqli_query($koneksi, "SELECT * FROM grade5 WHERE id_grade5 = '$_GET[id]' ");
		...
		else if ($_GET['hal'] == "hapus")
		{
			...
			$hapus = mysqli_query($koneksi, "DELETE FROM grade5 WHERE id_grade5 = '$_GET[id]' ");
			if($hapus){
				echo "<script>
...
<div class="container">
	...
	<h1 class="text-center">Form Siswa Kelas 5</h1>
	...
	<div class="card mt-3">
	  ...
	  <div class="card-body">
	    ...
	    <table class="table table-bordered table-striped">
	    	...
	    	<?php
	    		$no = 1;
	    		$tampil = mysqli_query($koneksi, "SELECT * from grade5 order by id_grade5 desc");
	    	...
	    	<tr>
				...
				<td><?=$data['tgllhr']?></td>
	    		<td>
	    			<a href="grade5.php?hal=edit&id=<?=$data['id_grade5']?>" class="btn btn-warning"> Update </a><br />
	    			<a href="grade5.php?hal=hapus&id=<?=$data['id_grade5']?>" 
